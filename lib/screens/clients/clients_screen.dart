...
class _ClientsScreenState extends State<ClientsScreen> {
  ...
  Future<void> edit([ClientModel? client]) async { final ok = await showDialog<bool>(context: context, builder: (_) => ClientDialog(client: client, repo: repo)); if (ok == true && mounted) reload(); }
  Future<void> detail(ClientModel client) async { await Navigator.of(context).push(MaterialPageRoute(builder: (_) => ClientDetailScreen(client: client))); if (mounted) reload(); }
  Future<void> remove(ClientModel client) async {
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(title: const Text('Supprimer le client ?'), content: Text('Supprimer « ${client.name} » ? Les clients liés à des factures ne peuvent pas être supprimés.'), actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Annuler')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Supprimer'))]));
    if (ok != true) return;
  ...
  @override Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: Text(l10n.clients)), body: Column(children: [
      ...
      Expanded(child: FutureBuilder<List<ClientModel>>(future: future, builder: (context, s) {
        ...
        return RefreshIndicator(onRefresh: () async { reload(); await future; }, child: ListView.separated(padding: const EdgeInsets.all(16), itemCount: list.length, separatorBuilder: (_, _) => const SizedBox(height: 8), itemBuilder: (_, i) {
          ...
          return Card(child: ListTile(leading: const CircleAvatar(child: Icon(Icons.business_outlined)), title: Text(c.name), subtitle: Text([ids, contact].where((x) => x.isNotEmpty).join('\n')), onTap: () => detail(c), trailing: PopupMenuButton<String>(onSelected: (v) => v == 'edit' ? edit(c) : remove(c), itemBuilder: (_) => const [PopupMenuItem(value: 'edit', child: Text('Modifier')), PopupMenuItem(value: 'delete', child: Text('Supprimer'))])));
        }));
      }))
